...
int main()
{
    ...
    int N_ASTEROIDS = 1001; // Increased to add 100 more asteroids, plus the existing one
    ...
    orbital_entities[9] = { 300.0e9, 100.0e9, 0.0, 10.0e3, 5.0e3, 0.0, 1.0e20 }; // An asteroid (original one)

    // Add 100 more asteroids
    std::mt19937 gen(0); // Standard pseudo-random number generator, seeded for reproducibility
    std::uniform_real_distribution<> pos_dist_x(2.0e11, 1.0e12); // Position X from 200B to 1T meters
    std::uniform_real_distribution<> pos_dist_y(-5.0e10, 5.0e10); // Position Y around 0 with some spread
    std::uniform_real_distribution<> vel_dist_x(-5.0e3, 5.0e3); // Velocity X
    std::uniform_real_distribution<> vel_dist_y(2.0e3, 15.0e3); // Velocity Y (orbital speed)
    std::uniform_real_distribution<> mass_dist(1.0e18, 5.0e20); // Mass

    for (int i = 0; i < 1000; ++i) {
        double x = pos_dist_x(gen);
        double y = pos_dist_y(gen);
        double vx = vel_dist_x(gen);
        double vy = vel_dist_y(gen);
        double mass = mass_dist(gen);
        orbital_entities[10 + i] = { x, y, 0.0, vx, vy, 0.0, mass };
    }
    ...
    std::cout << "Tiempo de ejecución: " << t_final - t_inicial << " segundos" << std::endl;
    return 0;
}